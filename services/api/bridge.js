...
class proxyClientApi extends EventEmitter {
  constructor(emitter) {
    ...
    emitter.on('muted', function (d) { api.amit('muted', d.channel, d.status); });
    this.types = types;
  }
...
class proxyServerApi extends EventEmitter {
  ...
  muted(channel, status) { this.emitter.send({ event: 'muted', data: { channel, status }, replyExpected: false }); }
  silence() {
